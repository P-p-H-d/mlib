...
// Build a Single-Producer/Multiple-Consumer snapshot type named "snapshot".
// Snapshots are technically shared registers of any type between threads designed to exchange data quickly and atomically 
// Technically, SPMC snapshots are 2+N_READERS buffers.
// One thread writes complete arrays, multiple threads can read the latest published one.
SNAPSHOT_SPMC_DEF(snapshot, int_array_t)
...
{
    m_thread_t t1, t2, t3;
    // Initialize snapshot storage, then start one producer and two consumers.
    snapshot_init(g_snapshot, 2);

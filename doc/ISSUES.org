...
* TODO #27 : Hash-Map with Worst Case property                  :ENHANCEMENT:

Specialize hashmap with O(ln(n)) worst case:
ased on chaining that points on a red/black tree with the 
Open Point: how to do GC of such object in an efficient way

Not a good idead: worst case will still be O(n) due to the hashmap
reallocation. Best to use a B+TREE directly in this case.
...
based on OA hash map of taggued pointer (needs space for tagging)
...
Reduce cache usage ==> Shall be faster.
...
New operator: ERROR_MODEL which value is 0 (abort on error), 1 (throw on error)
or 2 (return code on error).

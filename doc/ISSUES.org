...
*** lock free node pool :
 ...
 -- freelist_t : basic internal lockfree list used to construct more user friendly list (FIFO queue to avoid putting constraints on free & alloc at the same times + it is needed for reclaim part).
  ...
  + freelist_push (&list, &node) is {
    node.next := NULL
    old_head := node
    atomic_xchg(&list.head, &old_head)
    if (old_head != NULL)
       atomic_store(&old_head.next, node);
    else
       CAS(&old.tail, &old_head, node)
  }
  + freelist_pop(&list) return node is {
    tail := atomic_load(&list.tail);
    if (tail == NULL) return NULL;
    do {
        node := tail;
    } while (!CAS(&list.tail, &tail, node.next))
    if (list.tail == NULL)
       CAS(&list.head, &node, NULL);
    return node
  } 
  (Something seems off when correcting other entries - I feel some concurrency issues. Maybe a sentinell will be better ? Is it possible ?)
  + empty()
...
*** lf_list_t is a stack based list based on RCU:
  ...
  _push(list, data, lf_mt_t) is {
   ...
   # ABA problem shall not happen since node cannot be reused immeditalely

...
Due to the nature of the C language, 'type safe' means that a proper warning
shall be generated by the compiler in case of wrong type passed as
arguments to the functions (and not errors). You can also request
the compiler to transform all warnings into errors to get proper errors
too if you want.
